...
class DatasetPreparationConfig:
    # fmt: off
    dataset_family: str = "vqa-v2"                                # Dataset family to prepare
    ...
    root_dir: Path = Path(                                      # Path to root directory for storing datasets
        ...
        "/home/ubuntu/vlm_prune/vlm-evaluation/train2026"
    )

    # HF Hub Credentials (for LLaMa-2)
    hf_token: Union[str, Path] = Path("/home/ubuntu/vlm_prune/RLVLM/vlm-evaluation/.hf_token")              # Env Variable or Path to HF Token
...
def prepare(cfg: DatasetPreparationConfig) -> None:
    overwatch.info(
        f"Downloading and Preparing VLM Evaluation Dataset `{cfg.dataset_family}`")

    # Phase 1 :: Download & Extract Raw Data to `cfg.data_dir` / cfg.dataset_id / "download"
    overwatch.info(
        f"Phase 1 =>> Downloading & Extracting `{cfg.dataset_family}` to {cfg.root_dir / 'download'}")
    ...
    overwatch.info(
        f"Phase 2 =>> Building Index Dataset(s) for `{cfg.dataset_family}` at {cfg.root_dir / 'datasets'}")
