...
from vlm_train.models.interfaces import VLM, ImageProcessor, Tokenizer
...
class InstructBLIP(VLM):
    def __init__(
        ...
        **_: str,
        ...
        for trigger_string in ["true", "false", "yes", "no"]:
            token_idx_list = self.text_img_processor.tokenizer.encode(
                trigger_string, add_special_tokens=False)
            assert len(
                token_idx_list) == 1, f'String "{trigger_string}" is tokenized as more than one token!'
    ...
    def load(self) -> Tuple[nn.Module, Tokenizer, ImageProcessor]:
        ...
        with self.distributed_state.main_process_first():
            text_img_processor = InstructBlipProcessor.from_pretrained(
                self.hub_path)
    ...
    def get_prompt_fn(self, dataset_family: str = "vqa-v2") -> Callable[[str], str]:
        ...
        text_vqa_prompt_fn = self.get_vqa_chat_prompt_fn(
            uncertainty_aware=False, ocr_handling=True)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for idx in range(len(batch_input_ids)):
                if return_string_probabilities is None:
                    ...
                    gen_texts.append(self.text_img_processor.decode(
                        gen_ids, skip_special_tokens=True).strip())

                else:
                    # InstructBLIP will by default output lowercase answers...
                    return_string_probabilities = [
                        s.lower() for s in return_string_probabilities]
