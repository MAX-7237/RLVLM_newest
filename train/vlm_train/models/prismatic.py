...
from prismatic_vlms.prismatic.models import load

from train.vlm_train.models.interfaces import VLM


class PrismaticVLM(nn.Module, VLM):
    def __init__(
        self,
        model_family: str,
        model_id: str,
        ...
        **_: str,
        ...
        self.dtype = {"fp32": torch.float32, "fp16": torch.float16,
                      "bf16": torch.bfloat16}[load_precision]
        self.hf_token = hf_token
        self.ocr = ocr
        self.cache_dir = "/home/ubuntu/vlm_prune/RLVLM/cache"
        # Get Distributed State
        self.distributed_state = PartialState()

        # Load Model on GPU(s)
        self.model, self.llm_backbone, self.vision_backbone = self.load()

        # Set Default VQA Generation Configuration
        self.max_length = max_length
        self.temperature = temperature
        self.generate_kwargs = {
            "do_sample": False, "max_new_tokens": self.max_length, "temperature": self.temperature}
    ...
    def load(self):
        ...
        vlm = load(load_from, hf_token=self.hf_token, cache_dir=self.cache_dir)
        vlm.to(self.distributed_state.device, dtype=self.dtype)

        # Get Tokenizer and Image Processor
        llm_backbone, vision_backbone = vlm.llm_backbone, vlm.vision_backbone
        return vlm, llm_backbone, vision_backbone
    ...
    def get_prompt_fn(self, dataset_family: str = "vqa-v2") -> Callable[[str], str]:
        ...
        vqa_uncertain_prompt_fn = self.get_vqa_chat_prompt_fn(
        ...
        text_vqa_prompt_fn = self.get_vqa_chat_prompt_fn(
    ...
    def get_mc_prompt_fn(self) -> Callable[[str], str]:
        ...
        def mc_prompt_fn(question: str, choices: List[str]) -> str:
            ...
            assert len(
                choices) <= 26, "Too many answer choices vs. possible letters in the alphabet!"
            choice_str = "\n".join(
