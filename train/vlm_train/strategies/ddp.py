...
class DDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        assert isinstance(
            self.vlm, DDP), "save_checkpoint assumes VLM is already wrapped in DDP!"
        ...
        torch.save({"model": model_state_dicts,
                   "optimizer": optimizer_state_dict}, checkpoint_path)
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        overwatch.info(
            "Placing Entire VLM (Vision Backbone, LLM Backbone, Projector Weights) on GPU", ctx_level=1)
        ...
        self.vlm = DDP(self.vlm, device_ids=[
                       self.device_id], gradient_as_bucket_view=True)
